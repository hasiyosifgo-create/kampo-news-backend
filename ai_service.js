...
async function fetchPharmaSitesViaRSS() {
  ...
  const directSites = [
    ...
    { name: 'イスクラ産業', url: 'https://www.iskra.co.jp/news/' },
...
- category: この記事が特定の製品に関する「具体的なアクション・出来事（新発売、販売終了、自主回収、出荷調整、パッケージや成分の変更、添付文書の改訂など）」のニュースである場合は "product" を出力してください。（※注意：「添付文書の改訂」は重要な変更ニュースですが、単に既存製品の「添付文書をWebに掲載しました」といった案内や、製品の一般的なカタログ・説明ページなどは新たな出来事ではないため "news" または "UNRELATED" としてください。また、厚生労働省などの行政方針や制度変更（義務化など）のニュースは、特定の企業名が含まれていても "news" として扱ってください）。その他の一般的なニュース（業界動向、企業の決算、人事、一般的な医療コラムなど）の場合は "news" を、個人の体験談や考察、Noteなどのブログ記事の場合は "blog" を出力してください。
- summary: 記事の要約（3〜4文程度）。※もし記事が「漢方」「生薬」「東洋医学」に直接関連しないニュース（例：単なる「独自素材」「健康成分」などの一般的な健康食品・サプリの研究成果、一般的な西洋薬、風邪薬、一般的な化粧品・スキンケア・ヘアケア、単なる企業のお問い合わせページ、会社概要、採用情報、リンク集、企業理念、会社の沿革（歴史・あゆみ）などの固定ページ、単なる既存製品のカタログや添付文書の「掲載・案内」など）の場合は、必ず \`UNRELATED\` という文字列だけを出力してください。漢方薬や生薬に直接関わらない健康・美容成分の研究は弾いてください。（※ただし、「薬機法（医薬品医療機器等法）」や「漢方薬・生薬、およびサプリメント・健康食品等に関わる国の制度改定・義務化・ガイドライン」などの行政方針に関するニュースは業界全体に関わるため、UNRELATEDにせず必ず抽出してください。また「新年のご挨拶」や「社長・役員交代のご挨拶」といった時事的なご挨拶ニュースも抽出してください）。また、詐欺サイトやスパムと思われる場合も \`SPAM\` と出力してください。
- tags: 
